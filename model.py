...
class Ensemble:
    def __init__(
        self, regions=["left_eye", "right_eye", "mouth"], num_frames=6, verbose=True
    ):
    ...
    def fit_region(self, region, images, labels, names, multi_cwSaab_parm):
        ...
        if len(self.defakeHop) == 0:
            ...
            assert np.array_equal(
                names, self.names
            ), "the order of names are not aligned with previous regions"
            assert np.array_equal(
                labels, self.labels
            ), "the order of labels are not aligned with previous regions"
    ...
    def concatenate_features(self, regions=None, train_flag=True):
        ...
        if train_flag:
            ...
            names = self.predict_names
        return self.concatenate_frames_features(
            features, self.labels, names, train_flag=train_flag
        )
    ...
    def concatenate_regions_features(self, regions, train_flag):
        ...
        for region in regions:
            if train_flag:
                ...
                assert region in self.predict_features, (
                    "Please train " + region + " first"
                )
    ...
    def concatenate_frames_features(self, features, labels, names, train_flag):
        ...
        for idx, prob in enumerate(features):
            ...
            for i in range(self.num_frames):
                if (
                    idx + i < len(names)
                    and utils.vid_name(names[idx + i]) == cur_vid_name
                    and utils.frame(names[idx + i]) == cur_frame + i * 6
                ):
...
if __name__ == "__main__":
    ...
    model = Ensemble(
        regions=["left_eye", "right_eye", "mouth"], num_frames=6, verbose=True
    )
    ...
    multi_cwSaab_parm = dict(
        ...
        spatial_components=[0.9, 0.9, 0.9],
        n_jobs=4,
        verbose=True,
    )
    ...
    for region in model.regions:
        ...
        model.fit_region(
            region, train_images, train_labels, train_names, multi_cwSaab_parm
        )
