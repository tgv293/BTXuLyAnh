...
def extract_patch(img, point, patch_size):
    patch = img[
        int(point[1] - patch_size / 2) : int(point[1] + patch_size / 2),
...
def PatchExtractor(video_path, landmarks_path, output_dir, patch_size=32):
    ...
    if not os.path.exists(landmarks_path):
        print(f"Landmarks path does not exist: {landmarks_path}")
        return

    df = pd.read_csv(landmarks_path)
    print(f"CSV Loaded: {landmarks_path}, Number of entries: {len(df)}")

    # Remove leading/trailing whitespace from column names
    df.columns = df.columns.str.strip()

    # Check if the 'success' column exists after stripping
    if "success" not in df.columns:
        print("Error: 'success' column is not found.")
        return
    ...
    while cap.isOpened():
        ...
        if count % 6 == 0 and "success" in df.columns and len(df["success"]) > count:
            print(f"Processing frame {count}: Success = {df['success'].iloc[count]}")
            if df["success"].iloc[count] == 1:
                frame = frame[:, :, ::-1]
                frames.append(frame)
    ...
    cap.release()
    print(f"Total frames processed: {count}, Valid frames: {len(frames)}")
    ...
    for idx, frame in enumerate(frames):
        ...
        y = np.array(df.iloc[frame_number[idx], 299 + 68 : 299 + 68 * 2]).reshape(
            68, -1
        )
        z = np.ones(68).reshape(68, -1)
        landmarks = np.concatenate((x, y), axis=1)
        aligner = FaceAligner(
            desiredLeftEye=(0.35, 0.35), desiredFaceWidth=128, desiredFaceHeight=128 * 2
        )
        ...
        if len(regions_image) == len(regions):
            for i, region in enumerate(regions):
                filename = os.path.join(
                    output_dir, region, str(frame_number[idx]).zfill(4) + ".bmp"
                )
                img = regions_image[i]
                save(img, filename)
            filename = os.path.join(
                output_dir, "aligned_face", str(frame_number[idx]).zfill(4) + ".bmp"
            )
            np.save(
                os.path.join(
                    output_dir, "aligned_face", str(frame_number[idx]).zfill(4) + ".npy"
                ),
                affined_landmarks,
            )
